...
// ReadOnly: boot loader status for discriminating BootLoader or Application:
// set 1 by power on reset, clear 0 by software reset
...
// disable 5V->3.3V LDO: 0=enable LDO for USB and internal oscillator under 5V power,
// 1=disable LDO, V33 pin input external 3.3V power
...
// internal oscillator enable and original clock selection: 1=enable & select internal clock, 0=disable & select external clock
...
// ReadOnly: watch-dog timer overflow interrupt flag, cleared by reload watch-dog count or auto cleared when MCU enter interrupt routine
...
// ReadOnly: flash-ROM writing operation address valid flag, can be reviewed before or after operation: 0=invalid parameter, 1=address valid
...
//   1 1:  quasi-bidirectional (standard 8051 mode), open-drain output and input with pullup resistance,
//         just driving high level strongly for 2 clocks if turning output level from low to high
...
#define bT1_CT            0x40      // counter or timer mode selection for timer1: 0=timer, use internal clock, 1=counter,
                                    // use P3.5 (T1) pin falling edge as clock
...
#define bT0_CT            0x04      // counter or timer mode selection for timer0: 0=timer, use internal clock, 1=counter,
                                    // use P3.4 (T0) pin falling edge as clock
...
//   11: mode 3, TL0 is 8-bit timer or counter controlled by standard timer0 bits, TH0 is 8-bit timer using TF1 and controlled by TR1,
//               timer1 run enable if it is not mode 3
...
SFR(T2CON,      0xC8);    // timer 2 control
   ...
   SBIT(CP_RL2, 0xC8, 0);    // timer2 function selection (force 0 if RCLK=1 or TCLK=1): 0=timer and auto reload if count overflow or T2EX edge,
                             // 1=capture by T2EX edge
SFR(T2MOD,    0xC9);    // timer 2 mode and timer 0/1/2 clock mode
#define bTMR_CLK          0x80      // fastest internal clock mode for timer 0/1/2 under faster clock mode: 0=use divided clock,
...
#define bT2_CLK           0x40      // timer2 internal clock frequency selection: 0=standard clock, Fsys/12 for timer mode, Fsys/4 for UART0 clock mode,
                                    //   1=faster clock, Fsys/4 @bTMR_CLK=0 or Fsys @bTMR_CLK=1 for timer mode, Fsys/2 @bTMR_CLK=0 or
                                    // Fsys @bTMR_CLK=1 for UART0 clock mode
#define bT1_CLK           0x20      // timer1 internal clock frequency selection: 0=standard clock, Fsys/12, 1=faster clock,
                                    // Fsys/4 if bTMR_CLK=0 or Fsys if bTMR_CLK=1
#define bT0_CLK           0x10      // timer0 internal clock frequency selection: 0=standard clock, Fsys/12, 1=faster clock,
                                    // Fsys/4 if bTMR_CLK=0 or Fsys if bTMR_CLK=1
...
SFR(SPI0_STAT,       0xF8);    // SPI 0 status
   ...
   SBIT(S0_IF_BYTE,  0xF8, 4);    // interrupt flag for a byte data exchanged, direct bit address clear or write 1 to clear
                                  // or accessing FIFO to clear if bS0_AUTO_IF=1
...
#define bTKC_IF           0x80      // ReadOnly: interrupt flag for touch-key timer, cleared by writing touch-key control or
                                    // auto cleared when start touch-key checking
...
#define MASK_UIS_H_RES    0x0F      // ReadOnly: bit mask of current transfer handshake response for USB host mode: 0000=no response,
...
#define bUC_INT_BUSY      0x08      // enable automatic responding busy for device mode or automatic pause
...
//   1 1 1:  dual 64 bytes buffer by bUEP_R_TOG selection for receiving (OUT endpoint) + dual 64 bytes buffer by bUEP_T_TOG selection
//           for transmittal (IN endpoint), total=256bytes
...
//   1 0:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint) + 64 bytes buffer
//         for endpoint 4 receiving (OUT endpoint), total=128bytes
//   0 1:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint) + 64 bytes buffer
//         for endpoint 4 transmittal (IN endpoint), total=128bytes
...
#define bUH_R_RES         0x04      // prepared handshake response type for host receiving (IN): 0=ACK (ready), 1=no response,
                                    // time out to device, for isochronous transactions
...
#define bUH_T_RES         0x01      // expected handshake response type for host transmittal (SETUP/OUT): 0=ACK (ready), 1=no response,
